...
/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 start*/
#ifdef CONFIG_ARCH_X55
#include "custom_sysfs.h"
#endif
/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 end*/
		...
		void *display, u32 bl_lvl)
{
	...
	/*M55 code for SR-QN6887A-01-273 by hehaoran at 20231021 start*/
	DSI_INFO("bl_scale = %u, bl_scale_sv = %u, bl_lvl = %u\n",
		bl_scale, bl_scale_sv, (u32)bl_temp);
	/*M55 code for SR-QN6887A-01-273 by hehaoran at 20231021 end*/
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
			if (config->return_buf[i] !=
				...
				DRM_ERROR("return_buf[%d],mismatch: 0x%x, status_value[%d]= 0x%x\n",
		...
		struct dsi_display *display)
{
	...
	for (i = 0; i < count; ++i) {
		/*M55 code for QN6887A-191|1361 by yubo at 20231117 start*/
		#ifdef CONFIG_ARCH_X55
		flags = (cmds[i].msg.flags & BIT(0)) ? DSI_CTRL_CMD_READ : 0;
		#endif
		/*M55 code for QN6887A-191|1361 by yubo at 20231117 end*/
...
int display_get_tui_is_enabled(struct dsi_display *display) {
/*M55 code for P231107-03127 by hehaoran at 20240128 end*/
	struct sde_connector *sde_conn = NULL;
	struct sde_kms *sde_kms = NULL;

	if (display == NULL || display->drm_conn == NULL) {
		DSI_ERR("%s:invalid display or drm_conn\n",__func__);
		return -EFAULT;
	}

	sde_conn = to_sde_connector(display->drm_conn);
	if (sde_conn == NULL) {
		DSI_ERR("%s:invalid sde_conn\n",__func__);
		return -EFAULT;
	}

	sde_kms = _sde_connector_get_kms(&sde_conn->base);

	if (sde_kms == NULL) {
		DSI_ERR("%s:invalid sde_kms\n",__func__);
		return -EFAULT;
	}

	DSI_INFO("kms->trusted_display_enabled = %d\n", atomic_read(&sde_kms->trusted_display_enabled));

	return atomic_read(&sde_kms->trusted_display_enabled);
}
/*M55 code for P240111-04942 by hehaoran at 20240121 end*/
void dsi_display_read_cell_id(struct dsi_display *display, char *rx_buf)
{
	u8 *rx_tmp = NULL;
	int rx_len = 0;
	unsigned char buffer[CELL_ID_READ_CMD_LEN] = {0x0F, 0x06, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0xA1};

	DSI_INFO("%s:++++",__func__);
	/*M55 code for P240111-04942 by hehaoran at 20240121 start*/
	if (display_get_tui_is_enabled(display)) {
		memset(rx_buf, '0', buffer[0]);
		DSI_ERR("%s:tui can be enabled ,cant read cell_id!\n",__func__);
		return;
	}
	/*M55 code for P240111-04942 by hehaoran at 20240121 end*/
	...
	if (!rx_tmp) {
		DSI_ERR("%s:rx_tmp kzalloc mem failed!");
		return;
	}

	memset(rx_tmp, 0x0, MAX_CMD_RECEIVE_SIZE);
	rx_len = dsi_display_cmd_receive(display, buffer + 1, CELL_ID_READ_CMD_LEN - 1, rx_tmp, buffer[0]);
	if (rx_len <= 0 || strlen(rx_tmp) == 0) {
		DSI_ERR("%s read ddic cell id failed\n",  __func__);
		memset(rx_buf, '0', buffer[0]);
	} else {
		memcpy(rx_buf, rx_tmp, buffer[0]);
	}

	DSI_INFO("%s:rx_buf=%s",__func__,rx_buf);
	DSI_INFO("%s:-----",__func__);
	kfree(rx_tmp);
	rx_tmp = NULL;
}
...
/*M55 code for P240118-03676 by hehaoran at 20230124 start*/
#ifdef CONFIG_ARCH_X55
void dsi_display_read_bl_form_ddic(struct dsi_display *display, unsigned int *bl_lvl)
{
	u8 *rx_tmp = NULL;
	int rx_len = 0;
	unsigned int bl_tmp = 0;
	unsigned char buffer[BL_LVL_READ_CMD_LEN] = {0x02, 0x06, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x52};
	...
	memset(rx_tmp, 0x0, MAX_CMD_RECEIVE_SIZE);
	rx_len = dsi_display_cmd_receive(display, buffer + 1, BL_LVL_READ_CMD_LEN - 1, rx_tmp, buffer[0]);
	if (rx_len <= 0) {
		DSI_ERR("%s read ddic R52 bl_lvl failed\n",  __func__);
	} else {
		bl_tmp = (rx_tmp[0] << 8) + (rx_tmp[1] & 0xff);
		memcpy(bl_lvl, &bl_tmp, sizeof(bl_tmp));
		DSI_INFO("R52 bl_lvl =%d\n",*bl_lvl);
	}
...
/*M55 code for P240118-03676 by hehaoran at 20230124 end*/
...
/*M55 code for P240118-03676 by hehaoran at 20230127 start*/
#ifdef CONFIG_ARCH_X55
		if (display->panel->power_mode != SDE_MODE_DPMS_LP2) {
			//1.read R52 value from ddic
			dsi_display_read_bl_form_ddic(display, &display->panel->read_bl_lv);
		}
#endif//CONFIG_ARCH_X55
/*M55 code for P240118-03676 by hehaoran at 20230127 end*/
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 start*/
	#ifdef CONFIG_ARCH_X55
	rc = display_create_custom_sysfs(display);
	if (rc != 0) {
		DSI_ERR("create custom lcd sysfs fail\n");
	}
	#endif
	/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 end*/
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	if (!is_skip_op_required(display)) {
		...
		if (!display->poms_pending) {
			...
			/*M55 code for P240126-01496 by wenghailong at 20240219 start*/
			#ifdef CONFIG_ARCH_X55
			usleep_range(1000, 1500);
			rc = dsi_panel_reset_lock(display->panel);
			if (rc) {
			...
			#endif
			/*M55 code for P240126-01496 by wenghailong at 20240219 end*/
		}
	}
