...
// //M55 code for QN6887A-2993 by chengzhi at 2025/04/30 begin
#ifdef CONFIG_ARCH_X55
#include <linux/cpufreq.h>
#include <linux/cpu.h>
int hqextendCameraOpenCpuPolicyThread(void* data) {
	const int cpuIndexMax = 16;
    struct cpufreq_policy *cpuFreqPolicy[16] = {NULL};
	unsigned int oldCpufreq_policy_subpolicy[16] ={0};
	s32 oldCpufreq_policyConstraints_maxFreq_targetValue[16] = {0};
	unsigned int backupCpufreq_policy_subpolicy[16] ={0};
	s32 backupCpufreq_policyConstraints_maxFreq_targetValue[16] = {0};
	int currentCpuIndex=0;
    unsigned int cpu = 0;
	CAM_ERR(CAM_UTIL,"do cpu policy run E:");
    for_each_possible_cpu(cpu) {
        cpuFreqPolicy[currentCpuIndex] = cpufreq_cpu_get(cpu);
        if (cpuFreqPolicy[currentCpuIndex] == NULL || currentCpuIndex >cpuIndexMax) {
            CAM_ERR(CAM_UTIL,"[hqdebug-sluggish-cpu]can't get cpu:%d cpuFreqPolicy[currentCpuIndex],currentCpuIndex=%d", cpu,currentCpuIndex);
            continue;
        }
        //set cpu frequency to max
		// cpuFreqPolicy[currentCpuIndex]->max = cpuFreqPolicy[currentCpuIndex]->cpuinfo.max_freq;
		// CAM_ERR(CAM_UTIL,"run  set  CPU %u frequency to performance,cpuFreqPolicy[currentCpuIndex]->max=%ld", cpu,cpuFreqPolicy[currentCpuIndex]->max);
		// {
			// need set cpuFreqPolicy[currentCpuIndex]->max_freq_req->type to FREQ_QOS_MAX[2]
			// struct freq_constraints *pValue = cpuFreqPolicy[currentCpuIndex]->max_freq_req->qos; cpuFreqPolicy[currentCpuIndex]->max_freq_req->qos = &cpuFreqPolicy[currentCpuIndex]->constraints;
			// freq_qos_update_request(cpuFreqPolicy[currentCpuIndex]->max_freq_req, cpuFreqPolicy[currentCpuIndex]->max);//todo
			// cpuFreqPolicy[currentCpuIndex]->max_freq_req->qos = pValue;
		// }
		oldCpufreq_policy_subpolicy[currentCpuIndex] = cpuFreqPolicy[currentCpuIndex]->policy;
		oldCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex] =
				cpuFreqPolicy[currentCpuIndex]->constraints.max_freq.target_value;

		CAM_ERR(CAM_UTIL,"run set CPU %u set policy from %d ->%d , set target value from %d -> %d",
				cpu,oldCpufreq_policy_subpolicy[currentCpuIndex],CPUFREQ_POLICY_PERFORMANCE,
				oldCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex],
				cpuFreqPolicy[currentCpuIndex]->cpuinfo.max_freq);
		cpuFreqPolicy[currentCpuIndex]->policy = CPUFREQ_POLICY_PERFORMANCE;
		cpuFreqPolicy[currentCpuIndex]->constraints.max_freq.target_value = cpuFreqPolicy[currentCpuIndex]->cpuinfo.max_freq;
		refresh_frequency_limits(cpuFreqPolicy[currentCpuIndex]);
		CAM_ERR(CAM_UTIL,"done set  CPU %u frequency to performance,cpuFreqPolicy[currentCpuIndex]->max=%ld", cpu,cpuFreqPolicy[currentCpuIndex]->max);
		currentCpuIndex++;
    }
	CAM_ERR(CAM_UTIL,"do cpu policy run set finish,will restore after 4s:");
	//sleep 4s
    usleep_range(4000000, 4000000);
	cpu = 0;
	currentCpuIndex=0;
	CAM_ERR(CAM_UTIL,"do cpu policy run set finish,do restore");
    for_each_possible_cpu(cpu) {
        cpuFreqPolicy[currentCpuIndex] = cpufreq_cpu_get(cpu);
        if (cpuFreqPolicy[currentCpuIndex] == NULL || currentCpuIndex >cpuIndexMax) {
            CAM_ERR(CAM_UTIL,"[hqdebug-sluggish-cpu]can't get cpu:%d cpuFreqPolicy[currentCpuIndex],currentCpuIndex=%d", cpu,currentCpuIndex);
		...
		backupCpufreq_policy_subpolicy[currentCpuIndex] = cpuFreqPolicy[currentCpuIndex]->policy;
		backupCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex] =
				cpuFreqPolicy[currentCpuIndex]->constraints.max_freq.target_value;

		CAM_ERR(CAM_UTIL,"run restore CPU %u set policy from %d ->%d , set target value from %d -> %d",
				cpu,backupCpufreq_policy_subpolicy[currentCpuIndex],oldCpufreq_policy_subpolicy[currentCpuIndex],
				backupCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex],
				oldCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex]);
		cpuFreqPolicy[currentCpuIndex]->policy = oldCpufreq_policy_subpolicy[currentCpuIndex] ;
		cpuFreqPolicy[currentCpuIndex]->constraints.max_freq.target_value = oldCpufreq_policyConstraints_maxFreq_targetValue[currentCpuIndex];
		refresh_frequency_limits(cpuFreqPolicy[currentCpuIndex]);
		CAM_ERR(CAM_UTIL,"done restore  CPU %u frequency to performance,cpuFreqPolicy[currentCpuIndex]->max=%ld", cpu,cpuFreqPolicy[currentCpuIndex]->max);
		currentCpuIndex++;
    }
	CAM_ERR(CAM_UTIL,"do cpu policy run restore finish");
	CAM_ERR(CAM_UTIL,"do cpu policy run X:");
    return 0;
}

int hqdebug_cpuPerfFunc(void) {
	struct task_struct *threadHqextendCameraOpenCpuPolicyThread =NULL;
	CAM_ERR(CAM_UTIL,"do cpu policy init E:");
	threadHqextendCameraOpenCpuPolicyThread = kthread_run(hqextendCameraOpenCpuPolicyThread, NULL, "hqextendCameraOpenCpuPolicyThread");
	if (IS_ERR(threadHqextendCameraOpenCpuPolicyThread)) {
		CAM_ERR(CAM_UTIL,"can't create cpu policy thread:hqextendCameraOpenCpuPolicyThread");
	}
	CAM_ERR(CAM_UTIL,"do cpu policy init X:");
    return 0;
}
#endif
// //M55 code for QN6887A-2993 by chengzhi at 2025/04/30 end
    ...
    (struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    // else if(*ppos == 88)
    {
        CAM_INFO(CAM_UTIL,"[hqdebug-perf]E:main-process");
        hqdebug_cpuPerfFunc();
        CAM_INFO(CAM_UTIL,"[hqdebug-perf]X:main-process");
    }
	if(*ppos > 100)
	{
