...
pal_param_rotation_t param_angle;
...
int AudioDevice::CreateAudioPatch(audio_patch_handle_t *handle,
    ...
    switch (sources[0].type) {
        ...
        case AUDIO_PORT_TYPE_MIX: // Patch for audio playback
            ...
            if (old_device != AudioExtn::get_device_types(device_types)) {
                rx_device_switch = true;
            }
            old_device = AudioExtn::get_device_types(device_types);
    ...
    if (primary_out_io_handle == io_handle) {
        ...
        // call voice routesteam primary routing case
...
#ifdef SEC_AUDIO_COMMON
    static bool usb_connect_route = false;
    if ((voice_->mode_ == AUDIO_MODE_IN_COMMUNICATION) && usb_out_headset && !usb_input_dev_enabled &&
        (AUDIO_DEVICE_OUT_USB_HEADSET == AudioExtn::get_device_types(device_types))) {
        std::shared_ptr<StreamInPrimary> astream_in = GetActiveInStreamByUseCase(USECASE_AUDIO_RECORD_VOIP);
        if (astream_in && (AUDIO_DEVICE_IN_BUILTIN_MIC == AudioExtn::get_device_types(astream_in->mAndroidInDevices))) {
            /* For connect route */
            astream_in->RouteStream(astream_in->mAndroidInDevices, true);
            usb_connect_route = true;
        }
    } else if ((voice_->mode_ == AUDIO_MODE_IN_COMMUNICATION && !usb_out_headset && usb_connect_route)) {
        std::shared_ptr<StreamInPrimary> astream_in = GetActiveInStreamByUseCase(USECASE_AUDIO_RECORD_VOIP);
        if (astream_in && (AUDIO_DEVICE_IN_BUILTIN_MIC == AudioExtn::get_device_types(astream_in->mAndroidInDevices))) {
            /* For disconnect route */
            astream_in->RouteStream(astream_in->mAndroidInDevices, true);
            usb_connect_route = false;
        }
    }
#endif
...
#define VOICEMMODE1_VSID 0x11C05000
    ...
    adev_->call_type_realcalling = false;
    adev_->call_type_wificalling = false;
    adev_->call_type_volte_video = false;
    adev_->call_type_gsm3g_voice = false;
    /*M55 code for QN6887A-1679 by hujincan at 2023/12/06 start*/
    adev_->call_band_vowifi = NB;
    ...
    adev_->voip_via_smart_view = false;
#endif
...
int AudioDevice::SetParameters(const char *kvpairs) {
    ...
    if (ret >= 0) {
        ...
        /*M55 code for SR-QN6887A-01-563 by yingboyang at 20230919 start*/
        AHAL_DBG("%s rotation enter !",__func__);
        /*M55 code for SR-QN6887A-01-563 by yingboyang at 20230919 end*/
        switch (val) {
        case 270:
        {
    ...
    /*M55 code for SR-QN6887A-01-563 by yingboyang at 20230919 start*/
    //when playing music
    ret = str_parms_get_str(parms, "g_hw_display_rotation", value, sizeof(value));
    if (ret >= 0) {
        AHAL_DBG("%s g_hw_display_rotation value is :%s",__func__,value);

        if (strncmp(value, "0", 1) == 0) {
            param_angle.angle = 0;
            AHAL_DBG("%s fsm rotaion is :%d ",__func__,param_angle.angle);
        } else if (strncmp(value, "1", 1) == 0) {
            param_angle.angle = 90;
            AHAL_DBG("%s fsm rotaion is :%d ",__func__,param_angle.angle);
        } else if (strncmp(value, "2", 1) == 0) {
            param_angle.angle = 180;
            AHAL_DBG("%s fsm rotaion is :%d ",__func__,param_angle.angle);
        } else if (strncmp(value, "3", 1) == 0) {
            param_angle.angle = 270;
            AHAL_DBG("%s fsm rotaion is :%d ",__func__,param_angle.angle);
        } else {
            AHAL_ERR("error unexpected rotation of %s", value);
        }
        //Swap the speakers
        AHAL_DBG("fsm Swapping the speakers ");
        ret = pal_set_param(PAL_PARAM_ID_DEVICE_ROTATION,
                (void*)&param_angle,
                sizeof(pal_param_rotation_t));
        AHAL_DBG("fsm Speakers swapped ");
    }
    /*M55 code for SR-QN6887A-01-563 by yingboyang at 20230919 end*/
    ...
    /*M55 code for SR-QN6887A-01-565&SR-QN6887A-01-572&QN6887A-71 by yingboyang at 20230918 start*/
    ret = str_parms_get_str(parms, "SET_MIC_CHOOSE", value, sizeof(value));
    if (ret >= 0) {
        AHAL_DBG("mic test is open !");

        if (strncmp(value, "1", 1) == 0) {
            mic_test = true;
            mic_choose = MIC_TEST_MIC1_ONLY;
            AHAL_DBG("%s MIC_TEST_MIC1_ONLY :%d",__func__,mic_choose);
        } else if (strncmp(value, "2", 1) == 0) {
            mic_test = true;
            mic_choose = MIC_TEST_MIC2_ONLY;
            AHAL_DBG("%s MIC_TEST_MIC2_ONLY :%d",__func__,mic_choose);
        } else {
            mic_test = false;
            AHAL_DBG("MIC TEST VALUE IS:%s!", value);
        }
    }

    ret = str_parms_get_str(parms, "SET_SPEAKER_CHOOSE", value, sizeof(value));
    if (ret >= 0) {
        AHAL_DBG("speaker test is  open  !");

        if (strncmp(value, "1", 1) == 0) {
            speaker_test = true;
            speaker_choose = SPK_TEST_SPK1_ONLY;
            AHAL_DBG("%s SPK_TEST_SPK1_ONLY :%d",__func__,speaker_choose);
        } else if ( strncmp(value, "2", 1) == 0) {
            speaker_test = true;
            speaker_choose = SPK_TEST_SPK2_ONLY;
            AHAL_DBG("%s SPK_TEST_SPK2_ONLY :%d",__func__,speaker_choose);
        } else {
            speaker_test = false;
            AHAL_DBG("SPK TEST VALUE IS:%s!", value);
        }
    }

#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
    ret = str_parms_get_str(parms, AUDIO_PARAMETER_SEC_GLOBAL_VOICE_WAKEUP_REGISTER_VOICE_KEYWORD, value, sizeof(value));
    if (ret >= 0) {
        register_voice_keyword = atoi(value);
        str_parms_del(parms, AUDIO_PARAMETER_SEC_GLOBAL_VOICE_WAKEUP_REGISTER_VOICE_KEYWORD);
    }

    ret = str_parms_get_str(parms, AUDIO_PARAMETER_SEC_GLOBAL_VOICE_WAKEUP_SEAMLESS_ENABLE, value, sizeof(value));
    if (ret >= 0) {
        seamless_enabled = (strcmp(value, AUDIO_PARAMETER_VALUE_TRUE)) ? false : true;
        str_parms_del(parms, AUDIO_PARAMETER_SEC_GLOBAL_VOICE_WAKEUP_SEAMLESS_ENABLE);
    }
#endif

#ifdef SEC_AUDIO_SCREEN_MIRRORING
        ret = str_parms_get_str(parms, AUDIO_PARAMETER_SEC_LOCAL_VOIP_VIA_SMART_VIEW,value, sizeof(value));
        if (ret >= 0) {
            bool new_voip_via_smart_view = (strcmp(value, AUDIO_PARAMETER_VALUE_TRUE)) ? false : true;
            UpdateSmartViewState(new_voip_via_smart_view);
            str_parms_del(parms, AUDIO_PARAMETER_SEC_LOCAL_VOIP_VIA_SMART_VIEW);
        }
#endif

    /*M55 code for P231207-01565|P250317-00571 by hujincan at 2025/03/20 start*/
    ret = str_parms_get_str(parms, "l_voice_rx_control_mode", value, sizeof(value));
    if (ret >= 0) {
        bool voice_rx_mode = (strcmp(value, "1")) ? false : true;
        if (voice_rx_mode) {
            voice_->VoiceSetParameters("device_mute=true;direction=rx");
        } else {
            voice_->VoiceSetParameters("device_mute=false;direction=rx");
        }
        str_parms_del(parms, "l_voice_rx_control_mode");
    }

    ret = str_parms_get_str(parms, "l_screen_call", value, sizeof(value));
    if (ret >= 0) {
        int32_t os_version = property_get_int32("ro.build.version.release", 0);
        AHAL_INFO("os_version:%d", os_version);
        if (os_version == 14) {
            bool txt_call = (strcmp(value, "on")) ? false : true;
            if (txt_call) {
                voice_->VoiceSetParameters("device_mute=true;direction=rx");
            } else {
                voice_->VoiceSetParameters("device_mute=false;direction=rx");
        ...
        str_parms_del(parms, "l_screen_call");
    }
    /*M55 code for P231207-01565|P250317-00571 by hujincan at 2025/03/20 end*/

    /*M55 code for P240214-00183 by hujincan at 2024/02/28 start*/
    ...
    /*M55 code for P240214-00183 by hujincan at 2024/02/28 end*/
...
char* AudioDevice::GetParameters(const char *keys) {
    ...
    ret = str_parms_get_str(query, AUDIO_PARAMETER_SEC_LOCAL_DUAL_SPEAKER_AMP_LEFT_POWER_ENABLE, value, sizeof(value));
    if (ret >= 0) {
        strlcpy(value, (receiver_speaker_off ? AUDIO_PARAMETER_VALUE_FALSE : AUDIO_PARAMETER_VALUE_TRUE), sizeof(value));
        str_parms_add_str(reply, AUDIO_PARAMETER_SEC_LOCAL_DUAL_SPEAKER_AMP_LEFT_POWER_ENABLE, value);
    }
#endif
...
#ifdef SEC_AUDIO_COMMON
std::shared_ptr<StreamInPrimary> AudioDevice::GetActiveInStreamByUseCase(int UseCase) {
    std::shared_ptr<StreamInPrimary> astream_in = NULL;
    in_list_mutex.lock();
    for (int i = 0; i < stream_in_list_.size(); i++) {
        if (stream_in_list_[i]->mInitialized
          && (stream_in_list_[i]->GetUseCase() == UseCase)) {
            AHAL_VERBOSE("Found existing in stream by usecase");
            astream_in = stream_in_list_[i];
            break;
        }
    }
...
bool AudioDevice::RerouteForVoip() {
    bool ret = 0;
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance();
    ...
    if (astream_in) {
        astream_in->ForceRouteStream({AUDIO_DEVICE_NONE});
    }
    if (astream_out &&
...
#endif
#ifdef SEC_AUDIO_SCREEN_MIRRORING
void AudioDevice::UpdateSmartViewState(bool new_state) {
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance();
    if (new_state != voip_via_smart_view) {
        adevice->voip_via_smart_view = new_state;

        pal_param_speaker_status_t param_speaker_status;
        if (adevice->voip_via_smart_view) { // mute > routing
